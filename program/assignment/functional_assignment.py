...
class FunctionalAssignment(Assignment):
    ...
    def __init__(self, var, func, argument):
        ...
        if not self.argument.is_Symbol and not self.argument.is_Number:
            raise FunctionalAssignmentException(
                f"Argument to functional assignment must be number or symbol but is {self.argument}"
            )
    ...
    def evaluate_right_side(self, state):
        arg_value = (
            float(self.argument) if self.argument.is_Number else state[self.argument]
        )
        if self.func == "Sin":
    ...
    def get_const_moment(self, k: int):
        ...
        raise FunctionalAssignmentException(
            f"The function {self.func} is not supported."
        )
    ...
    def get_func_moment(cls, dist: Distribution, func_powers: Dict[str, int]):
        ...
        if is_trig_moment and is_exp_moment:
            raise FunctionalAssignmentException(
                "Exponential and trigonometric moments cannot be mixed"
            )
        ...
        for k1 in range(cos_power + 1):
            for k2 in range(sin_power + 1):
                if id_power == 0:
                    ...
                    cf_term = diff(dist.cf(t), t, id_power).xreplace(
                        {t: 2 * (k1 + k2) - cos_power - sin_power}
                    )
                result += (
                    math.comb(cos_power, k1)
                    * math.comb(sin_power, k2)
                    * ((-1) ** (sin_power - k2))
                    * cf_term
                )
    ...
    def get_exp_moment(cls, dist: Distribution, func_powers: Dict[str, int]):
        ...
        if not dist.mgf_exists_at(exp_power):
            raise FunctionalAssignmentException(
                f"The exponential moment of {dist} of order {exp_power} does not exist."
            )
        if id_power == 0:
